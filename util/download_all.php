...
if(isset($_GET['list'])){
    ...
    $id_list = explode(',', $query);

    $sql = "SELECT * FROM XML WHERE xml_id = ";

    for($b = 0; $b < count($id_list); $b++){

        if($b+1 == count($id_list))
            $sql .= $id_list[$b];
        else
            $sql .= $id_list[$b] . ' OR xml_id = ';
    ...
    <sample>
        ...
        <verticalExtent>' . $vert . '</verticalExtent>
                    ...
                    //print $xmlOut;
                    $zip->addFromString($row["Title"] . '_' . $row["xml_id"] . '.xml', $xmlOut);
                }
            } else {
